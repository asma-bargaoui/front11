...
export class ManageFactureComponent implements OnInit {
  ...
  getAllFacture(config: { limit: number; offset: number }): void {
    this.factureService.getAllFacture(config).subscribe((data: any[]) => {
      ...
      this.items = data;
      // Set the totalItems property based on the total count of items from the backend API response
  ...
  calculateTotalItems(data: any[]): number {
    // Implement the logic to calculate the total count of items from the API response
    return data.length; // Replace this with the correct logic based on your API response
  }
